...
function ApplePodcastIcon(props: IconProps) {
  return (
    <svg aria-hidden="true" viewBox="0 0 32 32" {...props}>
      <path
        fillRule="evenodd"
...
function OvercastIcon(props: IconProps) {
  return (
    <svg aria-hidden="true" viewBox="0 0 32 32" {...props}>
  ...
  return (
    <AudioProvider>
      <div className="flex min-h-screen w-fill flex-col lg:flex-row">
        {/* Sidebar header - sticky on desktop, collapsible on mobile */}
            ...
            <div className="flex gap-8 text-foreground">
              {([
                ...
                <Link
                  ...
                  aria-label={label}
                >
                  <Icon className="h-6 w-6 fill-muted-foreground group-hover:fill-foreground" />
                </Link>
              ))}
            </div>
          </div>
          
          {/* Sidebar content - always visible on desktop, collapsible on mobile */}
          <div className="px-4 py-6 sm:px-6 lg:px-8">
            
            {/* Logo - desktop only (mobile version is in the header) */}
            <div className="hidden lg:block">
              <Link
                href="/"
                className="relative mx-auto block w-48 overflow-hidden rounded-lg bg-muted shadow-xl shadow-muted/20 sm:rounded-xl"
                aria-label="Homepage"
              >
                <Image
                  className="w-full"
                  ...
                  priority
                />
                <div className="absolute inset-0 rounded-lg ring-1 ring-inset ring-border sm:rounded-xl" />
              </Link>
            </div>
            
            {/* Title and description - both mobile and desktop */}
        ...
        <main className="flex-1 relative" suppressHydrationWarning>
          ...
          <div className="relative">
            {children}
            <div className="pb-28 lg:pb-32"></div>
          </div>
        </main>

        {/* Mobile footer */}
        <footer className="border-t border-border bg-muted py-10 pb-40 sm:py-16 sm:pb-32 lg:hidden">
          <div className="mx-auto px-4 sm:px-6 md:max-w-2xl md:px-4">
            <AboutSection />
            <h2 className="mt-8 flex items-center font-mono text-sm font-medium leading-7 text-foreground">
              <PersonIcon className="h-3 w-auto fill-muted-foreground" />
              <span className="ml-2.5">Powered by</span>
            </h2>
            <div className="mt-2 flex gap-6 text-sm font-bold leading-7 text-foreground">
